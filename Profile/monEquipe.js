...
export default function monEquipe( { route , navigation } ){
    ...
    const functionOfFetching = () => {
        setTimeout(() => {
            ...
            .then( () => setpanding(false))
            .catch((error) => console.error(error));
            fetch('https://massive-monkey-9.loca.lt/Equipe?id_chef_Equipe='+id)
            .then((response) => response.json())
    ...
    return (
        <TouchableWithoutFeedback onPress={ () => Keyboard.dismiss() } >
            <ImageBackground source={ require('../3132.jpg') } style={Styles.imag} >
                <KeyboardAwareScrollView refreshControl={ <RefreshControl onRefresh={onRefresh} refreshing={refreshing}/> } >
                    ...
                    >
                        <Text style={{ fontSize: 23, color: '#ff6701', fontWeight: "bold", textShadowColor: "grey", textShadowRadius: 8 }}>Mon Equipe</Text>
                    </View>
